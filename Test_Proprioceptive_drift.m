...
figure;
P2 = polyfit(Result(2:30,1)*3,Result(2:30,2)*3,3);
X2 = -14*3:0.1:14*3;
Y2 = polyval(P2,X2);
hold on;
plot(Result(:,1)*3,Result(:,2)*3,'.','color','k','markersize',20)
hold on;
L2 = plot(X2,Y2,'linewidth',2,'color','k')
set(gca,'XLim',[-45 45]);
set(gca,'XTick',[-45:15:45]);
L3 = plot([-45,45],[0,0],'linewidth',2,'color','b')
L4 = plot([-45,45],[-45,45],'linewidth',2,'color','r')

xlabel('Disparity (deg)','fontsize',14);
ylabel('Drift (deg)','fontsize',14);
legend([L2,L3,L4],'Multisensory integration','Proprioceptive based','Vision based','Location', 'SouthEast')
set(gca,'fontsize',13)
